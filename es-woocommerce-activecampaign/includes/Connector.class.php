...
class AC_Connector {
	...
	public function curl($url, $params_data = array(), $verb = "", $custom_method = "") {
		...
		if ($this->debug) {
			$this->dbg($url, 1, "pre", "URL: ");
			$debug_str1 .= "I am here at File: ".__FILE__." Line #: ".__LINE__."\n";
		}
		if ($verb == "GET") {
			if (isset($data) && $data) {
				if ($this->debug) {
					$debug_str1 .= "I am here. File: ".__FILE__." Line #: ".__LINE__."\n";
					$this->dbg($data, 1, "pre", "Description: GET data");
					$this->dbg($url.$data, 1, "pre", "Description: url.data");
				}
				$response = wp_remote_get($url."&".$data, array('timeout' => 30));
			} else {
			...
			$response = wp_remote_post($url, array('method'=>$verb, 'body'=>$data, 'timeout' => 30));
		}

		// Trigger error to test purposes
		//$response = new WP_Error( 'toy', 'my favorite toy is dolly' );

		if ( is_wp_error( $response ) ) {
			$error_message = $response->get_error_message();
			//echo "es-woocommerce-activecampaign error: $error_message";
			set_transient("es_wc_activecampaign_errors", $error_message, 45);
			return "es-woocommerce-activecampaign error: $error_message";
		}

		$object = json_decode($response["body"]);
		if ($this->debug) {
			$this->dbg($response, 1, "pre", "Description: Response");
			$this->dbg($object, 1, "pre", "Description: Response object (json_decode)");
			$this->dbg(is_object($object), 1, "pre", "Description: is_object(object)");
		}
		if (!is_array($object) && (!is_object($object) || !isset($object->result_code))) {
			return $object->result_code." ".$object->result_message;
		}
		//$debug_str1 .= "curl_exec(\$ch);\n";
		if ($this->debug) {
			$this->dbg($object, 1, "pre", "Description: Raw response");
		}
		//$http_code = curl_getinfo($request, CURLINFO_HTTP_CODE);
		$http_code = $response["response"]["code"];
		if (!preg_match("/^[2-3][0-9]{2}/", $http_code)) {
			// If not 200 or 300 range HTTP code, return custom error.
			set_transient("es_wc_activecampaign_errors", "HTTP code $http_code returned", 45);
		...
		if ( !is_object($object) || (!isset($object->result_code) && !isset($object->succeeded) && !isset($object->success)) ) {
			//echo "<p>Line #: ".__LINE__." http_code = ".$http_code." method = ".$method."</p>";
			...
			$requestException = new ESWC\AC\Exception\RequestException;
